...
if is_g2pw:
    ...
    g2pw = G2PWPinyin(model_dir="/data1/ziyiliu/tts/GPT-SoVITS/GPT_SoVITS/text/G2PWModel",model_source=os.environ.get("bert_path","/data1/ziyiliu/tts/GPT-SoVITS/GPT_SoVITS/pretrained_models/chinese-roberta-wwm-ext-large"),v_to_u=False, neutral_tone_with_five=True)
...
def g2p(text):
    print(f"text before g2p: {text}")
    pattern = r"(?<=[{0}])\s*".format("".join(punctuation))
    sentences = [i for i in re.split(pattern, text) if i.strip() != ""]
    print(f"text after g2p: {sentences}")
...
def _g2p(segments):
    ...
    for seg in segments:
        pinyins = []
        print(f"seg before custom pinyin: {seg}")
        seg, customer_pinyin = custom_pinyin(seg)
        print(f"seg after custom pinyin: {seg}")
        print("customer_pin: ", customer_pinyin)
